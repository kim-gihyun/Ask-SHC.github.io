import { FREE_MODELS, MODEL_CATALOG } from './models.ts';
export { FREE_MODELS } from './models.ts';
...
export async function completeWithFallback(key:string,messages:ChatMessage[],preferred:string=FREE_MODELS[0],request:typeof fetch=fetch,options:{onAttempt?:(attempt:Attempt)=>void;signal?:AbortSignal;validateAnswer?:(answer:string)=>boolean}={}):Promise<CompletionResult>{
 ...
 for(const model of models){
  ...
  const record=(status:number,reason:string)=>{const attempt={model,status,reason,attemptedAt};attempts.push(attempt);options.onAttempt?.(attempt);};
  if(options.signal?.aborted)throw new DOMException('Request cancelled','AbortError');
  const mandatoryReasoning=MODEL_CATALOG.find(m=>m.id===model)?.mandatoryReasoning;
  try{
   const response=await request('https://openrouter.ai/api/v1/chat/completions',{
    ...
    body:JSON.stringify({model,messages,temperature:.15,max_tokens:mandatoryReasoning?4096:1800,reasoning:mandatoryReasoning?{enabled:true}:{enabled:false},provider:{max_price:{prompt:0,completion:0}}}),signal:options.signal?AbortSignal.any([options.signal,AbortSignal.timeout(12000)]):AbortSignal.timeout(12000),
   });
   ...
   if(response.ok&&!data.error){
    ...
    if(typeof answer==='string'&&answer.trim()&&data.choices?.[0]?.finish_reason!=='length'){
     if(options.validateAnswer&&!options.validateAnswer(answer)){record(status,'invalid_citations');continue;}
   ...
   if(daily){record(status,'daily_quota');return {ok:false,error:'OpenRouter’s daily free-model allowance for this account has been reached. Switching models cannot reset that shared allowance. Please try again after the quota resets; the source excerpts are still available.',code:'daily_quota',status:429,attempts};}
   const contextLimit=status===400&&/context|token|maximum.*length/.test(detail);
   const canFallback=status===429||status>=500||[400,404,408,422].includes(status)||contextLimit;
  ...
  }catch(error){if(options.signal?.aborted)throw error;record(0,'timeout_or_network');}
 }
 const rateLimited=attempts.some(a=>a.status===429);
 return {ok:false,error:`None of the ${attempts.length} free models tried could complete a supported answer just now. You can still read the matching source excerpts.`,code:rateLimited?'providers_busy':'providers_unavailable',status:rateLimited?429:503,attempts};
}
