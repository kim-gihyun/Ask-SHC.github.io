...
export function tokens(text:string){return (text.toLowerCase().match(/[a-z0-9]+/g)||[]).filter(t=>t.length>1&&!stop.has(t)).map(t=>t.replace(/(ing|ies|s)$/,(s)=>s==='ies'?'y':''));}
export function retrieve(query:string,docs:Document[],limit=6):Chunk[]{
 const synonyms:Record<string,string>={guest:'visitor',guests:'visitors',fridge:'refrigerator',wifi:'internet network',washing:'laundry',rent:'lodging fees',quiet:'noise',leave:'check out',move:'check in',gym:'fitness',parcel:'mail',packages:'mail',cost:'fees charges',apply:'admission application',readmission:'readmission admission'};
