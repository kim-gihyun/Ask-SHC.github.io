...
Each question attempts the configured primary model, then the other allowed free model after a provider rate limit, context/output limit, empty response, timeout or service failure. Previous failures do not silently skip either model. Successful primary answers stop the sequence. Shared account daily quotas, authentication and account restrictions stop immediately because changing models cannot resolve them. The local eight-questions-per-minute limit still applies.

The chat displays an ordered Model attempts panel with model names, results and timestamps. It expands automatically for fallback and document-only responses. API responses include the same attempt records; server logs contain only these diagnostic fields, not keys or questions. Token usage does not demonstrate whether a rejected request was attempted. Thirteen fallback and source-extraction tests pass, including both switch directions and fresh retries after prior failures.
